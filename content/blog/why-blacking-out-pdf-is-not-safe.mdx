...
PDF files are not single images. They contain multiple layers:

| PDF component | What it holds | Redaction risk if ignored |
|---------------|---------------|---------------------------|
| Content streams | Text drawing operators (`Tj`, `TJ`) | Text remains searchable under black boxes |
| Fonts | Character code mappings | Extractors can rebuild strings |
| Images | Scanned page bitmaps | Visual-only cover leaves OCR text |
| Annotations | Comments, highlights, shapes | Separate from visible page content |
...
| Marketing term | What users think | What often happens |
|----------------|------------------|----------------------|
| Highlight | Temporary mark | Reversible yellow overlay |
| Redact | Permanent removal | True in pro tools only if applied |
| Black shape | Hidden forever | Cosmetic cover — text may remain |
...
| Technique | How it works | Security level |
|-----------|--------------|----------------|
| Content stream removal | Deletes text objects and fonts in marked areas | High — verify anyway |
| Rasterization (flattening) | Page becomes bitmap; text operators gone | High — [RedactPDF](/tool) uses this on marked pages |
| Sanitization | Strips metadata, hidden layers, attachments | Medium — complements visual redaction |
| Black shape overlay | Draws on top without deleting | **Unsafe** — fails search/copy tests |
...
[RedactPDF](/privacy-policy) answers: permanent text removal in boxes; HTTPS apply, no stored copies.
